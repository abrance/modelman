...
本仓库**不负责部署**。不要在这里写 SSH、容器编排或云主机相关的逻辑。
...
所有服务必须实现 `GET /`（服务落点，两个服务都返回自带页面）、`/livez`、`/healthz`、
`/readyz`、`/version`、`/models`、`/metrics`（需鉴权），以及各自的业务端点；
错误响应必须带可读的 `error` 字段。

字段级语义、鉴权列与错误码约定**以 `docs/design.md` 的「服务契约」为唯一权威**，
两边不要各维护一份表（`tools/docs_check.py` 会校验两边的端点集合一致）。
...
动到文档时再加 `make docs-check`（CI 的 lint job 会跑）。
...
4. 凡是占用磁盘或内存的逻辑，都必须能指出限制它的那个数值：大输入设体积与
   尺寸上限、解码走带限制的路径；落盘的东西（临时文件、缓存、状态卷）要么有
   容量上限，要么有回收机制。写不出数值就不写这条路径。
5. 阈值、并发、尺寸上限等参数走环境变量，不要硬编码。

## 安全与隐私

1. 服务不带账号体系；能否被公网访问由部署仓库（`cops`）决定，当前两个服务的入口
   都公网可达且未启用 `AUTH_TOKEN`（`docs/design.md` D15–D17，统一鉴权另行做）。
