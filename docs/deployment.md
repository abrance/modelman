...
- **OCR（9101）**：拍照/传图 → 文字。设计与交互见 `docs/ocr-ui.md`。
- **日志聚类（9103）**：贴日志 → 看模板。设计与交互见 `docs/logcluster-ui.md`。

页面和 API 在同一个端口上，所以开放页面就是把那个端口暴露出去。

**当前的决定是两个都不带鉴权就开**（`docs/design.md` D15、D16、D17）。但代价不一样：

- **OCR**：`/ocr` 是纯 CPU 消耗型接口，被滥用表现为共享主机的额度被占满。上界
  （`MAX_CONCURRENCY`、`QUEUE_TIMEOUT_SECS`、compose 的 `cpus`/`memory`）保证对方
  拿到 503 而不是主机过载。
- **日志聚类**：`/cluster` 是**写**接口 —— 页面上的「聚类」按钮谁都能按，按下去就改
  模板树，而模板污染不会自动恢复（只能清空状态卷重学，代价是丢掉已累积的模板）。
  把页面挂出去，等于把"改模板树"这件事交给任何能访问入口的人。

挂入口时要确认的三件事两个服务一样：

1. **用 HTTPS。** HTTP 下浏览器不给非安全上下文用 `navigator.clipboard`，OCR 页面的
   「一键复制」会直接失效；将来若开鉴权，凭据也会明文过网。
2. **入口必须落在域名根路径。** 页面用绝对路径调 `/ocr` 与 `/cluster`，所以入口要把
   `https://<域名>/` 转发到对应的回环端口；挂在子路径（如 `https://host/model-ocr/`）
   下页面能打开但取不到数据。
