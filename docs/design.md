...
| D15 | OCR 的页面（`/`）与 `/ocr` 对公网**不启用鉴权**就开放（与 D14 里日志聚类的同类选择共享前提） | 目的是先把手机→页面的链路跑通，不想先拉上密钥下发那一套。代价明确：任何能访问入口的人都能调 `/ocr`，消耗的是共享云主机的 CPU。影响面被已有上界卡住（`MAX_CONCURRENCY=2`、`LIMIT_CONCURRENCY=8`、compose 里 `cpus=2/memory=1500M`），被滥用时表现为对方拿到 503，而不是拖垮主机。回退成本极低：页面与服务端都已实现并测过 token，以后要收敛只需改 `cops` 一处（`SECRET_ENV` + `deploy.yml` 映射 + 云主机密钥文件），不用改代码、不用重新设计 | 已接受（有意为之） |
...
| OCR 的页面（`/`）与 `/ocr` 将不带鉴权对公网开放 | 能访问入口的人都能调用 CPU 密集接口；入口一旦被扫描到就可能被持续占用 | 有意接受（D15）。上界已由 `MAX_CONCURRENCY=2`、`LIMIT_CONCURRENCY=8`、compose 的 `cpus=2/memory=1500M` 卡住，滥用表现为 503 而不是主机过载；要收敛时启用 `AUTH_TOKEN` 即可，两侧代码已就绪 |
