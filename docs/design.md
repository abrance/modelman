...
| 端点 | 语义 | 鉴权 |
|---|---|---|
| `GET /` | 服务落点。ocr 返回自带 Web 页面（`text/html`，200）；logcluster 尚未补齐（直接探根路径还是 404）。反代与外部探活器常直接探根路径，所以这里**不能没有响应**；需要 JSON 存活响应时用 `/livez` | 否 |
| `GET /livez` | 进程存活 | 否 |
...
| D17 | 日志聚类也自带页面（`/`），鉴权口径与 D16 一致：不启用 `AUTH_TOKEN` | 页面要有人真的用，靠 curl 贴日志不现实；页面本身不泄露数据，但它把 `/cluster` 这个**写**接口摆到了入口后面 —— 「打开域名的人能改模板树」从纸面变成顺手的事。页面上把「聚类」（写）与「只匹配」「看模板」（读）分开，写入按钮明确标注代价，让操作的人自己按下决定；回退与 D16 同一处（启用 `AUTH_TOKEN`，页面已支持填 token） | 已接受（有意为之） |
