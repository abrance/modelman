...
| D14 | 日志聚类上线后四项显式不做：不迁移 NAS 旧实例的历史模板、暂不启用 `AUTH_TOKEN`、状态卷不做自动备份、不停掉 NAS 上的旧实例 | 旧实例没有代码调用方（kuba 仓库里只有部署手记），所以没有调用方依赖它的模板；服务只绑回环，暂不需要 token；状态从日志里可以重新学出来，丢失的代价是多跑一遍而不是丢数据；旧实例暂时留着当对照 | 已落地 |
