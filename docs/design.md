...
| 端点 | 语义 | 鉴权 |
|---|---|---|
| `GET /` | 根路径存活探针，等价 `GET /livez`；反代与外部探活器直接探根路径时的默认落点（logcluster 尚未补齐） | 否 |
| `GET /livez` | 进程存活 | 否 |
...
| OCR 的 `/ui` 与 `/ocr` 将不带鉴权对公网开放 | 能访问入口的人都能调用 CPU 密集接口；入口一旦被扫描到就可能被持续占用 | 有意接受（D15）。上界已由 `MAX_CONCURRENCY=2`、`LIMIT_CONCURRENCY=8`、compose 的 `cpus=2/memory=1500M` 卡住，滥用表现为 503 而不是主机过载；要收敛时启用 `AUTH_TOKEN` 即可，两侧代码已就绪 |
