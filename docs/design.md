...
| 约束 | 实测值 | 对设计的影响 |
|---|---|---|
| 部署主机资源 | cloud2（已迁离）：4 核，3.6 GiB 内存；cloud3（现在）：16 GB 内存 / 68 GB 磁盘，无 GPU | 必须压内存占用；必须给并发设上界；必须显式设置线程数，不能留给运行时按核数推导 |
| 主机上的邻居 | cloud2 时代同机有 lems、ptdoc、qdrant、postgres 等（清单以 cops 仓库为准）；cloud3 目前只有 model 系列与 traefik | 资源限额与日志轮转是必需品，不是优化 |
| 境内网络 | cloud2 禁止直连 `ghcr.io`，走 `ghcr.chenby.cn`；cloud3 可直连 `ghcr.io` | 镜像 tag 必须不可覆盖：镜像站按 tag 缓存，覆盖后会拿到陈旧层 |
...
| 端口 | cloud2 上 11000 / 1502 / 4173 / 8081 / 9090 等已占用；模型服务用 91xx 段（9101/9103） | k3s 上容器统一 8080，对外端口由 Service/IngressRoute 决定 |
...
graph TD
  subgraph 交付层
    ...
    B -->|契约测试| C{门禁}
    C -->|通过| D[ghcr.io/abrance/modelman-*]
    C -->|失败| X[中止，不产出制品]
  end
  subgraph 运行层
    F[cops 仓库<br/>期望状态] -->|PR 合入 main| G[部署流程]
    D -->|拉镜像| H[k3s / compose<br/>等健康门禁]
    G --> H
    H --> I[model-ocr / model-logcluster<br/>Service + IngressRoute]
  end
  subgraph 调用层
    J[浏览器 / 上层应用] --> I
  end
...
| 运行层 | 保存期望状态并施加到主机（cops 仓库，部署模式与主机清单见其 README） | 不写模型逻辑，不编译代码 |
| 调用层 | 调用服务 | 不在本仓库范围内；入口的 TLS 由目标主机的 traefik 负责 |
...
| 端点 | 语义 | 鉴权 |
|---|---|---|
| `GET /` | 服务落点：两个服务都返回自带 Web 页面（`text/html`，200）。反代与外部探活器常直接探根路径，所以这里**不能没有响应**；需要 JSON 存活响应时用 `/livez` | 否 |
| `GET /livez` | 进程存活 | 否 |
...
| D9 | 服务不自带账号体系；不在本仓库里解决"谁能访问"：镜像监听 `0.0.0.0`，暴露面由部署侧决定（cloud2 用 compose 端口绑回环，k3s 用 Service/IngressRoute） | 服务没有账号体系，暴露面的控制权应该收在部署仓库，随部署模式变化 | 已落地；现状是不启用鉴权直接开（D15–D17），统一鉴权另行做 |
...
| D14 | 日志聚类上线时的三项显式不做：不迁移 NAS 旧实例的历史模板、状态卷不做自动备份、不停掉 NAS 上的旧实例（鉴权另见 D16） | 旧实例没有代码调用方（kuba 仓库里只有部署手记），所以没有调用方依赖它的模板；状态从日志里可以重新学出来，丢失的代价是多跑一遍而不是丢数据；旧实例暂时留着当对照 | 已落地；其中「不停旧实例」被 D16 取代（已停），模板树最终也随迁移清空（cloud3 上的 PVC 从空树开始） |
| D15 | OCR 的页面（`/`）与 `/ocr` 对公网**不启用鉴权**就开放（与 D14 里日志聚类的同类选择共享前提） | 目的是先把手机→页面的链路跑通，不想先拉上密钥下发那一套。代价明确：任何能访问入口的人都能调 `/ocr`，消耗的是共享云主机的 CPU。影响面被已有上界卡住（`MAX_CONCURRENCY=2`、`QUEUE_TIMEOUT_SECS=30`、compose 里 `cpus=2/memory=1500M`），被滥用时表现为对方拿到 503，而不是拖垮主机。回退成本极低：页面与服务端都已实现并测过 token，以后要收敛只需改 `cops` 一处（`SECRET_ENV` + `deploy.yml` 映射 + 云主机密钥文件），不用改代码、不用重新设计 | 已接受（有意为之） |
| D16 | 日志聚类同样不启用鉴权，并且停掉 NAS 上的旧实例 | 与 OCR 同一口径，理由是有意保持两个服务一致、少一套待维护的密钥。**代价比 OCR 重**：`/cluster` 是**写**接口，被滥用不只是白烧 CPU，而是任何人能在你的模板树里埋数据，而模板污染不会自动恢复（需要人工清空状态卷，代价是丢掉已累积的模板）。回退同样只需在 `cops` 开 `AUTH_TOKEN`，服务端逻辑已实现并有测试 | 已接受（有意为之） |
| D17 | 日志聚类也自带页面（`/`），鉴权口径与 D16 一致：不启用 `AUTH_TOKEN` | 页面要有人真的用，靠 curl 贴日志不现实；页面本身不泄露数据，但它把 `/cluster` 这个**写**接口摆到了入口后面 —— 「打开域名的人能改模板树」从纸面变成顺手的事。页面上把「聚类」（写）与「只匹配」「看模板」（读）分开，写入按钮明确标注代价，让操作的人自己按下决定；回退与 D16 同一处（启用 `AUTH_TOKEN`，页面已支持填 token） | 已接受（有意为之）。**注意这不是终局**：统一鉴权打算在外层单独做，届时服务侧的 `AUTH_TOKEN` 是保留还是删掉要重新定 |

## 已知风险

| 风险 | 影响 | 现状 |
|---|---|---|
| 两个服务都不启用鉴权就公开（页面 + API） | 能访问入口的人都能调接口；日志聚类还能往模板树里写数据，且污染不自愈 | 有意接受（D15–D17）。OCR 的滥用被并发上界卡住（503）；日志聚类的代价见 D16。统一鉴权已定为方向（外层做，见 roadmap 中期），届时重新评估服务侧 `AUTH_TOKEN` 的去留 |
| 单主机单副本，无冗余 | 目标主机故障即服务不可用（现集中在 cloud3 一台 k3s） | 可接受；回滚依赖重新部署到其它主机而非本机高可用 |
...
| 有状态服务（日志聚类）的状态不在镜像里 | 回滚镜像不会回滚状态；参数变更或状态损坏后需要人工处理 | 用 schema/profile 校验 + 降级 + 不覆盖旧状态兜住；卷不做自动备份（D14），丢失后重建或清空重学，见 `services/logcluster/README.md` 与 `docs/deployment.md` |
