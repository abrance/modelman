...
| D14 | 日志聚类上线后四项显式不做：不迁移 NAS 旧实例的历史模板、暂不启用 `AUTH_TOKEN`、状态卷不做自动备份、不停掉 NAS 上的旧实例 | 旧实例没有代码调用方（kuba 仓库里只有部署手记），所以没有调用方依赖它的模板；服务只绑回环，暂不需要 token；状态从日志里可以重新学出来，丢失的代价是多跑一遍而不是丢数据；旧实例暂时留着当对照 | 已落地（NAS 旧实例后已停，见 D16） |
...
| 风险 | 影响 | 现状 |
|---|---|---|
| 服务无鉴权，且位于反向代理之后 | 能访问入口的人都能调接口 | **已接受**（D15、D16）：两个服务都直接开放，不启用 `AUTH_TOKEN`。影响面由并发上界与 compose 限额卡住；要收敛时改 `cops` 三处即可，两侧代码已就绪 |
...
| OCR 的页面（`/`）与 `/ocr` 将不带鉴权对公网开放 | 能访问入口的人都能调用 CPU 密集接口；入口一旦被扫描到就可能被持续占用 | 有意接受（D15）。上界已由 `MAX_CONCURRENCY=2`、`QUEUE_TIMEOUT_SECS=30`、compose 的 `cpus=2/memory=1500M` 卡住，滥用表现为 503 而不是主机过载；要收敛时启用 `AUTH_TOKEN` 即可，两侧代码已就绪 |
