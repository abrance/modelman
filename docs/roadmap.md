...
- 与 OCR 的页面共用的是**形状**，不是代码：输入是贴日志文本或传日志文件，
  结果是模板列表与命中情况，没有图片与位置框。能复用的只有样式外壳（CSS 变量、
  卡片布局），`app.js` 基本要重写。
- 动之前要先定两件事：
  1. 页面挂在 `/` 意味着**任何打开域名的人都能往模板树里写数据**（`/cluster` 是写接口）。
     现在它没有入口，D16 的代价还只是纸面上的；挂上页面就看得见了。
  2. 样式外壳是与 OCR 各留一份，还是抽到 `shared/`。按 D12 的口径（服务之间只共享
     约定、不共享构建产物），各留一份更简单；等出现第三个页面再抽。
- 按惯例先出 `docs/logcluster-ui.md` 再动代码。
