...
- **不迁移 NAS 旧实例已经学出来的模板。** drain3 的状态跨脱敏配置不通用，
  ...
  本服务从空树开始。
- **不启用 `AUTH_TOKEN`。** 与 OCR 同一口径（`docs/design.md` D16）。注意 `/cluster`
  是写接口 —— 入口对外可达时，任何人都能往模板树里埋数据，而污染不会自动恢复，
  只能清空状态卷重学（代价是丢掉已累积的模板）。要收敛时在 `cops` 开 token 即可，
  服务端逻辑已实现并有测试，不用改代码。
- **不备份状态卷。** 模板可以从日志重新学出来，丢卷的代价是多跑一遍，不是丢数据。
- **NAS 上的旧实例已停**（D16）。现在 9103 是唯一实例；历史模板没迁过来，
  从空树重新累积。
